...
class Dynamics365Client:
    ...
    def find_active_case_by_subject(self, store_number, subject_code, case_type_code=None, preferred_case_type_code=None):
        """
        Check if an active case exists for the given store number with the
        same win_subject option set code (any date). Optionally also filter
        by casetypecode when case_type_code is provided.

        When preferred_case_type_code is provided and multiple active cases share
        the same subject, the one matching the preferred case type is returned.
        Falls back to the most recently created case if no type match is found.
        ...
        if case_type_code is not None:
            filter_expr += f" and casetypecode eq {case_type_code}"
        params = {
            "$filter": filter_expr,
            "$select": "incidentid,ticketnumber,win_storenumber,win_subject,createdon,_ownerid_value,casetypecode",
            "$top": 10,
            "$orderby": "createdon desc",
        }
        response = self._request("GET", "incidents", params=params)
        ...
        if preferred_case_type_code is not None and len(cases) > 1:
            selected = next(
                (c for c in cases if c.get("casetypecode") == preferred_case_type_code),
                cases[0],
            )
        else:
            selected = cases[0]

        owner_name = selected.get(
            "_ownerid_value@OData.Community.Display.V1.FormattedValue", ""
        )
        return {
            "case_id": selected.get("incidentid", ""),
            "ticketnumber": selected.get("ticketnumber", ""),
            "owner_name": owner_name,
        }

    def find_resolved_case_today_by_subject(self, store_number, subject_code, case_type_code=None, preferred_case_type_code=None):
        """
        Check if a resolved case exists for the given store number that was actually
        resolved today, with the same win_subject option set code. Optionally also
        filter by casetypecode when case_type_code is provided.

        When preferred_case_type_code is provided and multiple qualifying resolved cases
        share the same subject, the one matching the preferred case type is returned.
        Falls back to the first qualifying case if no type match is found.

        Uses incidentresolution activity createdon (not modifiedon on the incident)
        so that cases modified today for unrelated reasons (notes added, field edits, etc.)
        do not incorrectly match — only cases whose resolution activity was created today.
        ...
        # Step 1: Find candidate resolved cases for this store + subject (most recent first)
        filter_expr = (
            f"statecode eq 1 "
            f"and win_storenumber eq '{store_number}' "
            f"and win_subject eq {subject_code}"
        ...
        params = {
            "$filter": filter_expr,
            "$select": "incidentid,ticketnumber,_ownerid_value,casetypecode",
            "$top": 5,
            "$orderby": "modifiedon desc",
        }
        cases = self._request("GET", "incidents", params=params).json().get("value", [])
        if not cases:
            return None

        # Step 2: Navigate from each candidate incident to its incidentresolution activities
        # and check if any resolution was created today. Using the navigation property avoids
        # filtering by regardingobjectid directly on incidentresolutions (not supported).
        qualifying = []
        for case in cases:
            case_id = case.get("incidentid", "")
            res_params = {
                "$filter": (
                    f"createdon ge {today_start_utc} "
                    f"and createdon lt {today_end_utc}"
                ),
                "$select": "activityid",
                "$top": 1,
            }
            resolutions = self._request(
                "GET", f"incidents({case_id})/Incident_IncidentResolutions", params=res_params
            ).json().get("value", [])
            if resolutions:
        ...
        if preferred_case_type_code is not None and len(qualifying) > 1:
            selected = next(
                (c for c in qualifying if c.get("casetypecode") == preferred_case_type_code),
                qualifying[0],
            )
        else:
    ...
    def lookup_account_by_phone(self, phone_input):
        """
        Look up a single active account by phone number.

        Normalizes the input to 10 digits and tries all common stored formats
        (dashes, parentheses, spaces, dots, plain digits) against both
        telephone1 and telephone2 on active accounts.

        If the input is 11 digits and starts with '1' (US country code), the
        leading '1' is stripped automatically before the lookup.

        Returns:
            The matching account dict if exactly one match is found.
            None if no active account matches.
        Raises:
            ValueError if input does not normalize to 10 digits (after stripping).
            ValueError if multiple accounts match (caller must not guess).
        """
        ...
        if len(digits) != 10:
            raise ValueError(
                f"Phone '{phone_input}' does not normalize to 10 digits "
                f"(got {len(digits)})."
            )

        area, exch, num = digits[:3], digits[3:6], digits[6:]
        formats = [
            f"{area}-{exch}-{num}",      # 803-509-8588
            f"({area}) {exch}-{num}",    # (803) 509-8588
            f"{area} {exch} {num}",      # 803 509 8588
            f"{digits}",                  # 8035098588
            f"{area}.{exch}.{num}",      # 803.509.8588
        ...
        matched = {}  # accountid -> account dict (deduplicates across variants)
        for fmt in formats:
            for field in ["telephone1", "telephone2"]:
                params = {
                    "$filter": f"statecode eq 0 and {field} eq '{fmt}'",
                    "$select": "accountid,name,accountnumber,telephone1,telephone2",
                    "$top": "5",
                }
                try:
                    resp = self._request("GET", "accounts", params=params)
                    for acct in resp.json().get("value", []):
                        matched[acct["accountid"]] = acct
                except Exception:
                    pass  # Single format failure should not abort the whole search
        ...
        raise ValueError(
            f"Phone '{phone_input}' matched {len(matched)} accounts: "
            + ", ".join(
                f"store {a.get('accountnumber')} ({a.get('name')})"
                for a in matched.values()
            )
            + ". Cannot determine correct account."
        )
